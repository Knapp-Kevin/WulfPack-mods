...
## No Map placement and toggle contract

Rune Compass is anchored in the top-right HUD region where the minimap normally lives.

A persistent instrument toggle in that region is part of the intended shared UI contract with Celestial Dial. The toggle must remain available even when the minimap surface is absent in No Map play.

**That persistent toggle is a product requirement, not a claim about the current runtime.** It is not yet wired into `CompassSkin` or the current HUD implementation.
...
The current evidence includes a clean build, local bearing/interference assertions, lifecycle containment, live rendering, and all 13 operator protocol rows passing. Details are recorded in [STATUS.md](STATUS.md).
