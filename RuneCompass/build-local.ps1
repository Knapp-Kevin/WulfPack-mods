<#
.SYNOPSIS
    Build, install, disable, enable, uninstall, or inspect Rune Compass.
.DESCRIPTION
    Local helper only. This repository uses zero GitHub Actions. The script
    manages Rune Compass and nothing else.
#>
...
$PluginLeaf = "RuneCompass"
$ConfigLeaf = "com.wulfpack.runecompass.cfg"
...
function Invoke-Build {
    ...
    dotnet build (Join-Path $PSScriptRoot "RuneCompass.csproj") -c Release | Out-Host
    if ($LASTEXITCODE -ne 0) { throw "Rune Compass build failed." }
    $dll = Join-Path $PSScriptRoot "bin\Release\netstandard2.1\RuneCompass.dll"
    if (-not (Test-Path -LiteralPath $dll)) { throw "Build succeeded but RuneCompass.dll was not found." }
...
function Show-Status {
    param($Paths)
    $state = if (Test-Path -LiteralPath (Join-Path $Paths.Enabled "RuneCompass.dll")) { "INSTALLED (enabled)" }
             elseif (Test-Path -LiteralPath (Join-Path $Paths.Disabled "RuneCompass.dll")) { "INSTALLED (disabled)" }
             else { "NOT INSTALLED" }
    Write-Host ""
    Write-Host "Rune Compass : $state"
    ...
    if (Test-Path -LiteralPath $Paths.Log) {
        $hit = Select-String -LiteralPath $Paths.Log -Pattern "Rune Compass" -ErrorAction SilentlyContinue | Select-Object -Last 1
...
switch ($PSCmdlet.ParameterSetName) {
    "Install" {
        ...
        Write-Host "Installed Rune Compass."
    ...
    "Uninstall" {
        ...
        Write-Host "Uninstalled Rune Compass. Removed $count owned path(s)."
        Show-Status -Paths $paths
    }
