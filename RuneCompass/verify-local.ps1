...
# ----------------------------------------------------------- dead config ----
#
# Every accessor on CompassSettings must be BOTH wired by a config class and consumed by
# something that renders. This gate exists because IndependentLayerInterference silently
# became decoration: the model that read it was replaced, the helper holding the read was
# deleted with it, and every arithmetic assertion kept passing while the setting did nothing.
# Testing the maths never catches that. Only tracing the value does.

Write-Host ""
Write-Host "=== Config surface is live ==="

$settingsSrc = Get-Content -LiteralPath (Join-Path $PSScriptRoot "CompassSettings.cs") -Raw
$allSrc = (Get-ChildItem -LiteralPath $PSScriptRoot -Filter *.cs |
    Where-Object { $_.Name -ne "CompassSettings.cs" } |
    ForEach-Object { Get-Content -LiteralPath $_.FullName -Raw }) -join "`n"

$accessors = [regex]::Matches($settingsSrc, "public\s+Func<[^>]+>\s+(\w+)\s*=") |
    ForEach-Object { $_.Groups[1].Value }
$dead = @()
foreach ($a in $accessors) {
    # Two legitimate consumption idioms, and the gate must know both or it reports a
    # false positive on the second: invoked directly (settings.Foo()), or handed on as a
    # delegate for something else to invoke later (settings.Foo passed to a constructor).
    $invoked = $allSrc -match ("\." + [regex]::Escape($a) + "\s*\(\)")
    $passed = $allSrc -match ("\." + [regex]::Escape($a) + "\s*[,\)]")
    if (-not ($invoked -or $passed)) { $dead += $a }
}
if ($dead.Count -eq 0) {
    Write-Host ("  [PASS] {0,-46} = all {1} invoked" -f "every settings accessor is consumed", $accessors.Count)
} else {
    Write-Host ("  [FAIL] {0,-46} = {1}" -f "settings accessors nothing reads", ($dead -join ", ")) -ForegroundColor Red
    Write-Host "         A setting that nothing invokes is decoration; it will not do what its description promises." -ForegroundColor Red
    $script:Failures.Add("dead config accessor")
}
