<#
.SYNOPSIS
    Build, install, disable, enable, uninstall, or inspect Pied Piper.
.DESCRIPTION
    Local helper only. This repository uses zero GitHub Actions. The script
    manages Pied Piper and nothing else.
#>
...
$PluginLeaf = "PiedPiper"
$ConfigLeaf = "com.wulfpack.piedpiper.cfg"
...
function Invoke-Build {
    param($Paths)
    if (-not (Get-Command dotnet -ErrorAction SilentlyContinue)) { throw ".NET SDK not found on PATH." }
    $env:VALHEIM_MANAGED = $Paths.Managed
    $env:BEPINEX_CORE = $Paths.Core
    dotnet build (Join-Path $PSScriptRoot "PiedPiper.csproj") -c Release | Out-Host
    if ($LASTEXITCODE -ne 0) { throw "Pied Piper build failed." }
    $dll = Join-Path $PSScriptRoot "bin\Release\netstandard2.1\PiedPiper.dll"
    if (-not (Test-Path -LiteralPath $dll)) { throw "Build succeeded but PiedPiper.dll was not found." }
...
function Show-Status {
    param($Paths)
    $state = if (Test-Path -LiteralPath (Join-Path $Paths.Enabled "PiedPiper.dll")) { "INSTALLED (enabled)" }
             elseif (Test-Path -LiteralPath (Join-Path $Paths.Disabled "PiedPiper.dll")) { "INSTALLED (disabled)" }
             else { "NOT INSTALLED" }
    Write-Host ""
    Write-Host "Pied Piper   : $state"
    ...
    if (Test-Path -LiteralPath $Paths.Log) {
        $hit = Select-String -LiteralPath $Paths.Log -Pattern "Pied Piper" -ErrorAction SilentlyContinue | Select-Object -Last 1
...
switch ($PSCmdlet.ParameterSetName) {
    "Install" {
        ...
        Copy-Item -LiteralPath $dll -Destination (Join-Path $paths.Enabled "PiedPiper.dll") -Force
        if (Test-Path -LiteralPath $paths.Disabled) {
            Remove-OwnedPath -Path $paths.Disabled -ExpectedLeaf $PluginLeaf | Out-Null
        }
        Write-Host "Installed Pied Piper."
    ...
    "Uninstall" {
        ...
        Write-Host "Uninstalled Pied Piper. Removed $count owned path(s)."
        Show-Status -Paths $paths
    }
